...
def Clustering_Iteration(adata, ndims=30, min_pct=0.4, min_log2_fc=2, batch_size=256, min_score=150, min_de_genes=1, min_cluster_size=4, batch_key=None):
    ...
    for cluster in clusters:
        ...
        try:
            ...
            sc.pp.highly_variable_genes(cluster_adata, n_top_genes=n_genes, subset=False, flavor='seurat', layer='counts',
                                        span=0.5)
        except (ValueError, RuntimeError) as e:
            # If seurat_v3 fails (e.g., LOESS singularities), run with 'seurat' flavor
            print(f"Warning: seurat HVG failed for cluster {cluster} ({str(e)}), running seurat flavor instead")
